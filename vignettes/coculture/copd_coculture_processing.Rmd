...
sample_names <- sample_names[sample_names %in% c("AC20","AC21")]

# Define base path
base_path <- "/data/ac2215/unprocessedData/HBEC_KOs_all_lanes"

# Define sample-level metadata
selected_samples <- c("AC20", "AC21")
HBEC_donors <- c("COPD1", "HBEC1")
fib_donors <- c("COPD", "COPD")
...
```{r}
if(file.exists("/data/jx303/hlri_summer/data/copd_coculture_anchors.RData")){
  
  load(file = "/data/jx303/hlri_summer/data/copd_coculture_anchors.RData")
    ...
    save(anchors, file = "/data/jx303/hlri_summer/data/copd_coculture_anchors.RData")

}
```

```{r}
if(file.exists("/data/jx303/hlri_summer/data/copd_coculture_int.RData")){
  
  load(file = "/data/jx303/hlri_summer/data/copd_coculture_int.RData")
  
  }else{
    
    libs.combined <- IntegrateData(anchorset = anchors)

    save(libs.combined, file = "/data/jx303/hlri_summer/data/copd_coculture_int.RData")
...
libs.combined$orig.ident <- as.factor(libs.combined$orig.ident)
libs.combined <- ScaleData(libs.combined, vars.to.regress = c('percent.mt', "orig.ident"))
...
libs.combined <- FindClusters(libs.combined, resolution = 0.4, random.seed = 1996)
