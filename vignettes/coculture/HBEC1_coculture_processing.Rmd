---
title: "HBEC1 coculture processing"
author: "Joanna Xie"
date: "`r doc_date()`"
output:
...
sample_names <- sample_names[sample_names %in% c("AC01","AC04", "AC07")]

# Define base path
base_path <- "/data/ac2215/unprocessedData/HBEC_KOs_all_lanes"

# Define sample-level metadata
selected_samples <- c("AC01","AC04", "AC07")
HBEC_donors <- c("HBEC1", "HBEC1", "HBEC1")
fib_donors <- c("CAF", "COPD", "NA")
...
lapply(1:length(seurat_list_filtered), function(i) {
  ...
  plot1 + plot2 + patchwork::plot_annotation(title = names(seurat_list_filtered[i]))
  
})
```

```{r}
if(file.exists("/data/jx303/hlri_summer/data/HBEC1WT_coculture_anchors.RData")){
  
  load(file = "/data/jx303/hlri_summer/data/HBEC1WT_coculture_anchors.RData")
    ...
    save(anchors, file = "/data/jx303/hlri_summer/data/HBEC1WT_coculture_anchors.RData")

}
```

```{r}
if(file.exists("/data/jx303/hlri_summer/data/HBEC1WT_coculture_int.RData")){
  
  load(file = "/data/jx303/hlri_summer/data/HBEC1WT_coculture_int.RData")
  
  }else{
    
    libs.combined <- IntegrateData(anchorset = anchors)

    save(libs.combined, file = "/data/jx303/hlri_summer/data/HBEC1WT_coculture_int.RData")
...
```{r}
HBEC1WT_coculture_anno <- RenameIdents(libs.combined, 
                              "0" = "Suprabasal", 
                              "1" = "Suprabasal", 
                              "2" = "Ciliated",
                              "3" = "Basal",  
                              ...
                              "11" = "Secretory",
                              "12" = "Basal", 
                              "13" = "Proliferating Basal",
                              "14" = "Deuterosomal",
                              "15" = "Rare"
                              ) 

HBEC1WT_coculture_anno$cellType <- Idents(HBEC1WT_coculture_anno)
Idents(HBEC1WT_coculture_anno) <- "cellType"
```

```{r, fig.width = 8, fig.height = 6}
DimPlot(HBEC1WT_coculture_anno)
```
...
for (cell_type in names(list_of_cellmarkers)) {
  ...
  p <- VlnPlot(
    object = HBEC1WT_coculture_anno,
    features = markers,
    pt.size = 0
  )

  p <- p + patchwork::plot_annotation(title = paste("<b>Title</b>", cell_type, sep = " "))  
...
```{r}
saveRDS(HBEC1WT_coculture_anno, file = "/data/jx303/hlri_summer/data/HBEC1WT_coculture_annotated.Rds")
```



```{r, fig.width = 25}
p <- DotPlot(HBEC1WT_coculture_anno, features = list_of_cellmarkers) +
...
```{r}
DimPlot(HBEC1WT_coculture_anno, split.by = "orig.ident", ncol = 2)
DimPlot(HBEC1WT_coculture_anno, group.by = "orig.ident")
```
