---
title: "DKO coculture processing"
author: "Joanna Xie"
date: "`r doc_date()`"
output:
...
sample_names <- sample_names[sample_names %in% c("AC02","AC05", "AC15")]

# Define base path
base_path <- "/data/ac2215/unprocessedData/HBEC_KOs_all_lanes"

# Define sample-level metadata
selected_samples <- c("AC02","AC05", "AC15")
...
```{r}
if(file.exists("/data/jx303/hlri_summer/data/DKO_coculture_anchors.RData")){
  
  load(file = "/data/jx303/hlri_summer/data/DKO_coculture_anchors.RData")
    ...
    save(anchors, file = "/data/jx303/hlri_summer/data/DKO_coculture_anchors.RData")

}
```

```{r}
if(file.exists("/data/jx303/hlri_summer/data/DKO_coculture_int.RData")){
  
  load(file = "/data/jx303/hlri_summer/data/DKO_coculture_int.RData")
  
  }else{
    
    libs.combined <- IntegrateData(anchorset = anchors)

    save(libs.combined, file = "/data/jx303/hlri_summer/data/DKO_coculture_int.RData")
...
```{r}
saveRDS(DKO_coculture_anno, file = "/data/jx303/hlri_summer/data/DKO_coculture_annotated.Rds")
```
...
```{r}
DKO_coculture_anno <- RenameIdents(libs.combined, 
                              "0" = "Ciliated", 
                              "1" = "Basal", 
                              "2" = "Basal",
                              "3" = "Basal",  
                              ...
                              "14" = "Deuterosomal",
                              "15" = "Rare",
                              "16" = "Proliferating Basal", 
                              "17" = "Ciliated"
                              ) 

DKO_coculture_anno$cellType <- Idents(DKO_coculture_anno)
Idents(DKO_coculture_anno) <- "cellType"
```

```{r}
DimPlot(DKO_coculture_anno)
```


```{r, fig.width = 25}
p <- DotPlot(DKO_coculture_anno, features = list_of_cellmarkers) +
...
```{r}
DimPlot(DKO_coculture_anno, split.by = "orig.ident", ncol = 2)
DimPlot(DKO_coculture_anno, group.by = "orig.ident")
```

```{r}
DKO_coculture_anno <- readRDS("/data/jx303/hlri_summer/data/DKO_coculture_annotated.Rds")
DimPlot(DKO_coculture_anno, split.by = "orig.ident")
FeaturePlot(DKO_coculture_anno, feature= c("VIM", "COL1A1", "COL3A1", "ACTA2"))
```
