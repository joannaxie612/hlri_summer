---
title: "QKO coculture processing"
author: "Joanna Xie"
date: "`r doc_date()`"
output:
...
sample_names <- sample_names[sample_names %in% c("AC03","AC06", "AC09")]

# Define base path
base_path <- "/data/ac2215/unprocessedData/HBEC_KOs_all_lanes"

# Define sample-level metadata
selected_samples <- c("AC03","AC06", "AC09")
...
```{r}
if(file.exists("/data/jx303/hlri_summer/data/QKO_coculture_anchors.RData")){
  
  load(file = "/data/jx303/hlri_summer/data/QKO_coculture_anchors.RData")
    ...
    save(anchors, file = "/data/jx303/hlri_summer/data/QKO_coculture_anchors.RData")

}
```

```{r}
if(file.exists("/data/jx303/hlri_summer/data/QKO_coculture_int.RData")){
  
  load(file = "/data/jx303/hlri_summer/data/QKO_coculture_int.RData")
  
  }else{
    
    libs.combined <- IntegrateData(anchorset = anchors)

    save(libs.combined, file = "/data/jx303/hlri_summer/data/QKO_coculture_int.RData")
...
```{r}
QKO_coculture_anno <- RenameIdents(libs.combined, 
                              "0" = "Secretory", 
                              "1" = "Ciliated", 
                              "2" = "Suprabasal",
                              "3" = "Proliferating Basal",  
                              "4" = "Ciliated", 
                              "5" = "Secretory", 
                              "6" = "Basal" , 
                              "7" = "Secretory", 
...
QKO_coculture_anno$cellType <- Idents(QKO_coculture_anno)
Idents(QKO_coculture_anno) <- "cellType"
```

```{r, fig.width = 8, fig.height = 6}
DimPlot(QKO_coculture_anno)
```
```{r}
for (cell_type in names(list_of_cellmarkers)) {
  markers <- list_of_cellmarkers[[cell_type]]
...
```{r}
saveRDS(QKO_coculture_anno, file = "/data/jx303/hlri_summer/data/QKO_coculture_annotated.Rds")
```



```{r, fig.width = 25}
p <- DotPlot(QKO_coculture_anno, features = list_of_cellmarkers) +
...
```{r}
DimPlot(QKO_coculture_anno, split.by = "orig.ident", ncol = 2)
DimPlot(QKO_coculture_anno, group.by = "orig.ident")
```
